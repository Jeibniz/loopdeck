...
Work runs as **loops around the ticket queue**: _producers_ create work on their own cadence; a
_consumer_ drains it. Cadences live in a structured, hand-editable file — **`loops.yaml`** — so they're
...
Producers only _create_ `needs-triage` issues; you triage to `ready`; the consumer drains `ready`,
...
- **`/loop`** — repeats a command in a _live_ session; for attended draining (`/loop /autopilot`).
...
- Each cron run costs tokens — the crons _are_ the throttle; enable deliberately.
