export const meta = {
  name: 'review-panel',
  description: 'Review the current branch diff across multiple lenses, adversarially verify each finding, and return a deduped, confidence-checked report',
  phases: [
      ...
      findings.map((f) => () =>
        agent(
          `Adversarially verify this ${l.key} finding — try to REFUTE it. Default to 'refuted' if uncertain.\n` +
            `Finding: ${f.title}\nFile: ${f.file}:${f.line || '?'}\nDetail: ${f.detail}\n\n${scope}`,
          { label: `verify:${f.file}`, phase: 'Verify', agentType: 'verifier', schema: VERDICT_SCHEMA },
...
log(`Reviewed ${all.length} raw findings; ${confirmed.length} survived adversarial verify, ${all.length - confirmed.length} dropped.`);
