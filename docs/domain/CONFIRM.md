...
- **Fact:** vakio's `command:` is a multi-line folded scalar. The `yaml` lib preserves it automatically when you edit *other* keys, but editing `command` itself requires setting the Scalar node `type` to keep `>`.
...
- **Why it matters:** Add/delete-loop operations *could* nudge a trailing comment to an adjacent node in rare cases.
