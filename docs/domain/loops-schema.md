...
- **`routine:` is owned by `/loops apply`, not the human.** It's written back when a loop is reconciled into a `/schedule` cloud routine. **loopdeck must never drop or rewrite it** — surgical edits to *other* keys leave it intact. _Source:_ vakio loops.yaml inline comment "created by /loops apply 2026-06-15"; scaffold `loops` SKILL.md. _Confidence:_ high.
- **`command` may be a quoted scalar OR a folded/literal block scalar.** Editing a *different* key preserves the block style automatically; editing `command` itself must preserve `>`/`|` via the node `type` (see [[yaml-document-api]] §6b). _Confidence:_ high.
...
- `kind` is **optional** — vakio loops omit `consumer`; don't require it. Validate `kind ∈ {consumer,producer}` only *if present*.
